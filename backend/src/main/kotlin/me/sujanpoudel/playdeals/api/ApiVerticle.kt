...
class ApiVerticle(
  ...
  override suspend fun start() {
    ...
    router.route().handler(CorsHandler.create().addRelativeOrigin(config.cors))
    ...
    router.get("/").handler {
      ...
      BackgroundJobRequest.enqueue(uuid, AppDetailScrapJob.Request("$id"))
    ...
    log.info("API server running at : ${config.app.port}")
  }
}
