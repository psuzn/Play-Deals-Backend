...
class HealthTest(vertx: Vertx) : IntegrationTest(vertx) {
  ...
  @Test
  fun `GET liveness should return 200`() = runTest {
    val response = httpClient.get(8888, "localhost", "/health/liveness").send().await()
  ...
  @Test
  fun `GET readiness should return 200`() = runTest {
    val response = httpClient.get(8888, "localhost", "/health/readiness").send().await()
